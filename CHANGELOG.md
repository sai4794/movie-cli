...
- **cinestream year-tier + stable tie-break** — the year-less Cinemeta stub "Inception" (no streams) outranked the playable "Inception (2010)", and sort_by+reverse flipped the stable order among equal scores ("Inception Premiere" beat the film). Year-bearing records are now canonical and ties sort by original catalog order: inception→Inception (2010) first, pushpa→Pushpa: The Rise first, salaar→Salaar (2023) first.
- **vegamovies bundle posts** — multi-season bundles without direct per-season links (S1-5 packs) died with "No season links"; list_episodes now emits a Season pack entry so the title stays listable.
