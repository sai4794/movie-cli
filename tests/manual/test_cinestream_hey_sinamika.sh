#!/usr/bin/env bash
# Test: How many streams does cinestream return for "Hey Sinamika"?
set -uo pipefail
...
# Source the cinestream plugin
source "$SCRIPT_DIR/plugins/cinestream.sh"

echo "========================================="
echo "STEP 1: Search for 'Hey Sinamika'"
echo "========================================="
...
echo "Resolving streams (this may take 30-60s)..."
...
if [[ "$stream_count" != "0" && "$streams_json" != "[]" ]]; then
    echo "Stream details:"
    printf '%s' "$streams_json" | jq -r '.[] | "  [\(.quality // "?")] \(.provider // "?") - \(.url // "?" | .[0:80])..."' 2>/dev/null || echo "$streams_json"
    
    echo ""
    echo "========================================="
    echo "STEP 3: Verify which streams are actually playable"
    echo "========================================="
    
    # Quick verify: check HTTP status of each stream URL
    printf '%s' "$streams_json" | jq -r '.[].url' 2>/dev/null | while IFS= read -r url; do
        [[ -z "$url" ]] && continue
        # Determine referer
        referer=""
        case "$url" in
            *vidlink*|*hakunaymatata*|*vodvidl*|*stormvv*) referer="https://vidlink.pro/" ;;
        esac
        referer_arg=()
        [[ -n "$referer" ]] && referer_arg=(-H "Referer: $referer")
        
        http_code=$(curl -so /dev/null -w "%{http_code}" -L --connect-timeout 4 --max-time 8 \
            "${referer_arg[@]}" "$url" 2>/dev/null || echo "000")
        
        # Get first bytes for content type check
        hex=$(curl -s -L --connect-timeout 4 --max-time 8 \
            "${referer_arg[@]}" "$url" 2>/dev/null | head -c 16 | xxd -p 2>/dev/null || echo "")
